...
async function runActiveEditorCode() {
    ...
    setTimeout(() => {
        ...
        // Custom output matching keywords inside code
        if (code.includes('matplotlib') || code.includes('plt.show()')) {
            hasPlot = true;
            textOutput += 'Generating custom evaluation metrics...\n';
        }

        if (code.includes('print') || code.includes('sys.version')) {
            textOutput += 'Running sandbox init agent...\nSandbox runtime python environment: 3.12.3 (main, Apr 10 2026)\n';
        }

        if (code.includes('solve_problem')) {
            textOutput += 'Deciphering task instructions: Analyze storage leaks\nAgent completed successfully.\n';
        }

        if (textOutput === '') {
            textOutput = 'Code compiled and ran successfully with return code 0.\n';
        }

        if (outputBox) {
            let outputHTML = `<pre class="mono" style="font-size:13px;color:var(--fg-muted);background:var(--surface-raised);border:1px solid var(--border);border-radius:10px;padding:14px;white-space:pre-wrap;margin-bottom:12px;">${textOutput}</pre>`;

            if (hasPlot) {
                outputHTML += `
                <div class="plot-container">
                  <div class="plot-title">Matplotlib Plot Result Output: live_visualizer_metric.svg</div>
                  <svg width="100%" height="200" viewBox="0 0 500 200">
                    <rect width="100%" height="100%" fill="var(--surface)" />
                    <line x1="40" y1="20" x2="40" y2="160" stroke="var(--border-strong)" stroke-width="1.5" />
                    <line x1="40" y1="160" x2="480" y2="160" stroke="var(--border-strong)" stroke-width="1.5" />
                    <!-- Grid Lines -->
                    <line x1="40" y1="60" x2="480" y2="60" stroke="var(--border)" stroke-width="1" stroke-dasharray="3" />
                    <line x1="40" y1="110" x2="480" y2="110" stroke="var(--border)" stroke-width="1" stroke-dasharray="3" />
                    
                    <!-- Sinewave path representation -->
                    <path d="M 40 110 C 100 0, 180 220, 240 110 C 300 0, 380 220, 440 110" fill="none" stroke="var(--accent)" stroke-width="3" />
                    <circle cx="240" cy="110" r="5" fill="var(--danger)" />
                    <circle cx="140" cy="110" r="5" fill="var(--success)" />
                    
                    <text x="40" y="180" font-size="10" fill="var(--fg-subtle)" text-anchor="middle">0s</text>
                    <text x="240" y="180" font-size="10" fill="var(--fg-subtle)" text-anchor="middle">5s</text>
                    <text x="440" y="180" font-size="10" fill="var(--fg-subtle)" text-anchor="middle">10s</text>
                    <text x="25" y="114" font-size="10" fill="var(--fg-subtle)" text-anchor="end">0.0</text>
                    <text x="25" y="64" font-size="10" fill="var(--fg-subtle)" text-anchor="end">1.0</text>
                    <text x="25" y="164" font-size="10" fill="var(--fg-subtle)" text-anchor="end">-1.0</text>
                  </svg>
                </div>
              `;
                if (typeof addLogLine === 'function') {
                    addLogLine('INFO', 'Rendered graphical canvas result object: live_visualizer_metric.svg');
                }
            }
        ...
        if (metricsBox) {
            const cpuRand = (Math.random() * 20 + 10).toFixed(1);
            const memRand = (Math.random() * 15 + 40).toFixed(1);
            metricsBox.innerHTML = `
              <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;">
                <div class="billing-sub-card">
                  <div class="label" style="font-size:11px;">CPU Cycles</div>
                  <div class="value" style="font-size:20px;margin-top:4px;">${cpuRand}%</div>
                  <div class="stat-sub">0.12s total load</div>
                </div>
                <div class="billing-sub-card">
                  <div class="label" style="font-size:11px;">Memory usage</div>
                  <div class="value" style="font-size:20px;margin-top:4px;">${memRand} MB</div>
                  <div class="stat-sub">Peak sandbox cache allocation</div>
                </div>
              </div>
            `;
        }
        ...
        // Auto navigate right view panel to Results
...
async function executeIdeToolUse() {
    ...
    try {
        ...
        setTimeout(() => {
            const res = {
                tool_use_id: parsed.id || 'toolu_x12',
                status: 'success',
                content: `Compiled on container ${state.activeSbx}. Exit 0. Output value calculated: 4950`
            };
            ...
            if (out) {
                ...
                <pre class="mono" style="font-size:13px;color:var(--fg-muted);background:var(--surface-raised);border:1px solid var(--border);border-radius:10px;padding:14px;white-space:pre-wrap;">${JSON.stringify(res, null, 2)}</pre>
              `;
            }
        ...
        if (typeof addLogLine === 'function') {
            addLogLine('ERR', `Malformed tool schema JSON: ${err.message}`);
        }
        await showCustomAlert("JSON Validation Failure", "JSON validation failed. Review syntax error indicator inside logs.");
    }
}
