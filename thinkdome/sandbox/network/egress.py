...
All outbound network traffic from sandboxes is routed through this proxy layer under thinkdome.sandbox.network.egress.
The proxy enforces:
...
class EgressProxy:
    ...
    def evaluate(
        ...
        for idx, (rule, pattern) in enumerate(zip(self._rules, self._compiled_patterns)):
            if pattern.search(domain) and method_upper in rule.methods:
                matched_rule = rule
                matched_idx = idx
                break
        ...
        if not matched_rule:
            ...
            self._log_audit(url, method, domain, decision, sandbox_id, diagnostics_service)
            return decision
    ...
    def _log_audit(
        ...
        """Log egress decision for audit trail, system logs, and per-sandbox diagnostics."""
        entry = {
            "timestamp": time.time(),
            "sandbox_id": sandbox_id or "global",
            "url": url,
            "method": method,
            "domain": domain,
        ...
        # 1. Console / System Structured Log
        status_icon = "✅ ALLOWED" if decision.allowed else "⛔ DENIED"
        logger.info(
            f"🌐 Egress [{status_icon}] sandbox={sandbox_id or 'global'} {method} {url} — {decision.reason}"
        )

        # 2. Record to sandbox Diagnostics Service if provided
        if diagnostics_service and sandbox_id:
            try:
                diagnostics_service.record_event(
                    sandbox_id=sandbox_id,
                    event_type="network.egress",
                    message=f"{method} {url} -> {status_icon}",
                    details={
                        "domain": domain,
                ...
                logger.warning(f"Could not record network event to diagnostics_service: {e}")
